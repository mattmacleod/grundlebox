...
class EventTest < ActiveSupport::TestCase
  ...
  context "events with one performance" do
    setup do
      ...
      @event = Factory(:event)
      @event.save
      @performance = Factory(:performance, :event => @event, :starts_at => Time::utc(2010, 1, 1, 9, 0), :venue => @venue, :price => "5")
      @event.reload
    end
    ...
    context "with an end time" do
      setup do
        @performance.update_attribute(:ends_at, Time::utc(2010,1,1,13,0))
        @event.reload
      end
      should "have correct cached times" do
    ...
    context "over >1 day" do
      setup { @performance.update_attribute(:ends_at, Time::utc(2010,1,5,13,0) ); @event.reload }
      should "have correct cached times" do
        assert_equal "9:00am – 1:00pm", @event.cached_times
      end
      should "have correct cached dates" do
      ...
      context "with a different end time" do
        setup { @performance.update_attribute(:ends_at, Time::utc(2010,1,5,15,0)); @event.reload }
        should "have correct cached times" do
      ...
      context "ending in a different month" do
        setup { @performance.update_attribute(:ends_at, Time::utc(2010,2,1,15,0)); @event.reload }
        should "have correct cached dates" do
          assert_equal "1 Jan–1 Feb", @event.cached_dates
        end
      end
      context "ending in a different year" do
        setup { @performance.update_attribute(:ends_at, Time::utc(2011,2,1,15,0)); @event.reload }
        should "have correct cached dates" do
          assert_equal "1 Jan–1 Feb", @event.cached_dates
        end
      end
  ...
  context "events with multiple performances" do
    ...
    setup do
      ...
      @performances = (1..10).map do |n|
        @performance = Factory(:performance, :event => @event, :starts_at => Time::utc(2010, 1, n, 9, 0), :venue => @venue, :price => "5")
      end
      @event.reload
    end
    ...
    context "which do not happen on Sundays" do
      setup { @performances[2].destroy && @performances[9].destroy; @event.reload }
      should "have correct cached dates" do
        assert_equal "1–9 Jan, not Sundays", @event.cached_dates
      end
    end
    context "which do not happen on weekends" do
      setup { @performances[1].destroy && @performances[2].destroy && @performances[8].destroy && @performances[9].destroy; @event.reload }
      should "have correct cached dates" do
        assert_equal "1–8 Jan, weekdays only", @event.cached_dates
      end
    end
    context "starting and ending in different months" do
      setup do
        @performances = (1..21).map do |n|
          @performance = Factory(:performance, :event => @event, :starts_at => Time::utc(2010, 1, n+10, 9, 0), :venue => @venue, :price => "5")
        end
        @performance = Factory(:performance, :event => @event, :starts_at => Time::utc(2010, 2, 1, 9, 0), :venue => @venue, :price => "5")
        @event.reload
      end
      should "have correct cached dates" do
        assert_equal "1 Jan – 1 Feb", @event.cached_dates
    ...
    context "and different start times" do
      setup do
        @performances.first.update_attribute(:starts_at, Time::utc(2010,1,1,10,0))
        @event.reload
      end
      should "have correct cached times" do
    ...
    context "and different end times" do
      setup do
        @performances.first.update_attribute(:ends_at, Time::utc(2010,1,1,13,0))
        @event.reload
      end
      should "have correct cached times" do
        assert_equal "times vary", @event.cached_times
  ...
  context "events with multiple performances dispersed in time" do
    ...
    setup do
      ...
      @performances = (1..5).map do |n|
        @performance = Factory(:performance, :event => @event, :starts_at => Time::utc(2010, 1, n*2, 9, 0), :venue => @venue, :price => "5")
      end
      @event.reload
    end
    should "have correct cached dates" do
    ...
    context "dispersed over two months" do
      setup do
        @performances.first.update_attribute(:starts_at, Time::utc(2010, 2, 1, 9, 0))
        @event.reload
      end
      should "have correct cached dates" do
  ...
  context "events with two performances starting on the same day and ending on different days" do
    setup do
      ...
      @event = Factory(:event)
      @event.save
      @p1 = Factory(:performance, :event => @event, :starts_at => Time::utc(2009,1,1,9,0), :ends_at => Time::utc(2009,1,2,9,0))
      @p2 = Factory(:performance, :event => @event, :starts_at => Time::utc(2009,1,1,9,0), :ends_at => Time::utc(2009,1,3,9,0))
      @event.reload
    end
    should "have correct cached dates" do
